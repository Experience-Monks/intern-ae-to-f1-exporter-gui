...
if ($.global.is === undefined) {
	$.global.is = function () {
		return function is(value, type) {
			...
			var ZeroArgumentsError = new Error("is() expects at least one value and optionally a variable number of type arguments");
			var IncorrectTypeError = new Error("if a type is supplied, it is expected to be a non-anonymous Function definition.");
			var IncorrectTypesError = new Error("types, if supplied, are expected to be non-anonymous Function definitions.");

			var value_type = typeof value === "undefined" ? "undefined" : _typeof(value);

			//Validate value argument
			if (arguments.length == 0) throw ZeroArgumentsError;

			//Type not supplied
			if (arguments.length < 2) {
				if (value_type === "number" && isNaN(value)) return false;
			...
			for (var i = 1; i < arguments.length; i++) {
				if (typeof type !== "function" || type.name.length === 0) throw arguments.length == 1 ? IncorrectTypeError : IncorrectTypesError;
			} //Test types
			for (var i = 1; i < arguments.length; i++) {
				var type = arguments[i];

				if (value_type === "string" && type === String) return true;else if (value_type === "boolean" && type === Boolean) return true;else if (value_type === "number" && type === Number && !isNaN(value)) return true;else if (value_type === "function" && type === Function) return true;else if (value instanceof type) return true;
			}
...
if ($.global.get === undefined) {
	$.global.get = function (is) {
		...
		//	Valid Types
		/************************************************************************/
		var Types = [CompItem, FolderItem, FootageItem, AVLayer, ShapeLayer, TextLayer, CameraLayer, LightLayer, Property, PropertyGroup];
		...
		function property_is_hidden(prop) {
			...
			if (is(prop, PropertyGroup)) return property_group_is_hidden(prop);
		...
		function property_group_is_hidden(pGroup) {
			...
			for (var i = 1; i <= pGroup.numProperties; i++) {
				...
				if (is(prop, Property) && !property_is_hidden(prop)) all_hidden = false;
			}
			return all_hidden;
		}
		...
		function types_include_layer(types) {
			for (var i = 0; i < types.length; i++) {
				var type = types[i];
				if (type === AVLayer || type === ShapeLayer || type === TextLayer || type === CameraLayer || type === LightLayer) return true;
			}
		...
		function types_include_item(types) {
			for (var i = 0; i < types.length; i++) {
				var type = types[i];
				if (type === FolderItem || type === CompItem || type === FolderItem) return true;
			}
		...
		function types_include_property(types) {
			for (var i = 0; i < types.length; i++) {
				var type = types[i];
				if (type === Property || type === PropertyGroup) return true;
			}
		...
		function create_selector(arg) {
			...
			if (is(arg, RegExp) && selector == null) {
				selector = function selector(input) {
					return input.name.match(arg) !== null;
				};
			}

			if (is(arg, Number) && selector == null) {
				selector = function selector(input) {
					return input.label === arg;
				};
			}
			...
			if (!is(selector, Function)) throw new Error("Minify mangled selection creation again.");
		...
		function get_all_of_types(types, context_arr) {
			...
			for (var i = app.project.items.length; i > 0; i--) {
				...
				if (do_items && types_match(types, item)) context_arr.push(item);
				if ((do_layers || do_props) && item instanceof CompItem) {
					for (var ii = item.numLayers; ii > 0; ii--) {
						var layer = item.layer(ii);
						if (do_layers && types_match(types, layer)) context_arr.push(layer);
						if (do_props) rec_properties_to_array(layer, types, context_arr);
					}
				}
			}
		}
		...
		function rec_properties_to_array(group, types, arr) {
			for (var i = group.numProperties; i > 0; i--) {
				...
				if (is(prop, PropertyGroup)) rec_properties_to_array(prop, types, arr);

				if (!property_is_hidden(prop) && types_match(types, prop)) arr.push(prop);
			}
		}
		...
		function properties_to_array(group, types, arr) {
			...
			for (var i = group.numProperties; i > 0; i--) {
				var prop = group.property(i);
				if (property_is_hidden(prop) && types_match(types, prop)) continue;
			}
		}
		...
		function create_context(types, context) {
			...
			if (context === undefined) get_all_of_types(types, context_arr);else if (types_match(types, context)) context_arr.push(context);else if (is(context, ItemCollection, LayerCollection)) add_collection_to_array(context, types, context_arr);else if (has_properties(context) && types_include_property(types)) rec_properties_to_array(context, types, context_arr);else if (is(context, Array)) for (var i = 0; i < context.length; i++) {
		...
		function add_collection_to_array(collection, types, arr) {
			for (var i = collection.length; i > 0; i--) {
				...
				if (types_match(types, select)) arr.push(select);
			}
		}
			...
			for (var i = 0; i < args.length; i++) {
				...
				if (arg_is_type(arg)) {
					...
					for (var ii = 0; ii < args.length; ii++) {
						...
						if (arg_is_type(maybe_type)) types.push(maybe_type);else all_types = false;
					}
					if (all_types) do_splice = true;
				}

				if (do_splice) {
					//Remove types from args so they arn't reused when getting selector and context
					args.splice(i, 1);
					i--;
				}
			}

			if (types.length == 0) types = types.concat(Types);
		...
		function Query(args) {
			...
			this.selection = function (index) {
				...
				if (index_supplied) return selected[index];
		...
		Query.prototype.each = function (callback) {
			if (!is(callback, Function)) throw new Error(".each expects a function.");
		...
		Query.prototype.set = function (prop, value) {
			if (!is(prop, String)) throw new Error(".set expects the the first argument to be a string, representing a member name.");

			return this.each(function (select) {
				if (prop in select === false) return;

				var curr_value = is(value, Function) ? value(select) : value;

				if (!is(select[prop], Function)) select[prop] = curr_value;
			});
		};
		...
		Query.prototype.call = function (method /*args*/) {
			if (!is(method, String)) throw new Error(".call expects the the first argument to be a string, representing a method.");

			var args = Array.prototype.slice.call(arguments);
			args.shift();
			return this.each(function (select) {
				if (method in select === false || !is(select[method], Function)) return;
		...
		Query.prototype.children = function () /*args*/{
			...
			this.each(function (select) {
				if (is(select, FolderItem)) new_contexts.push(select.items);

				if (is(select, CompItem)) new_contexts.push(select.layers);

				if (has_properties(select)) rec_properties_to_array(select, [Property, PropertyGroup], new_contexts);
			});
